...
class CertifiedEsdfIntegrator {
 public:
  CertifiedEsdfIntegrator() = default;
  virtual ~CertifiedEsdfIntegrator();

  /// Build an CertifiedEsdfLayer from a CertifiedTsdfLayer
  /// @param tsdf_layer The input CertifiedTsdfLayer
  /// @param[out] esdf_layer The output CertifiedEsdfLayer
  void integrateLayer(const CertifiedTsdfLayer& tsdf_layer, CertifiedEsdfLayer* esdf_layer);
  ...
  /// Build an CertifiedEsdfLayer from a CertifiedTsdfLayer (incremental) (on GPU)
  /// @param tsdf_layer The input CertifiedTsdfLayer
  /// @param block_indices The indices of the CertifiedEsdfLayer which should be updated
  /// (usually because the TSDF at these indices has changed).
  /// @param[out] esdf_layer The output CertifiedEsdfLayer
  virtual void integrateBlocks(const CertifiedTsdfLayer& tsdf_layer,
                               const std::vector<Index3D>& block_indices,
                               CertifiedEsdfLayer* esdf_layer);

  /// @brief Build an CertifiedEsdfLayer from an OccupancyLayer(incremental) (on GPU)
  /// @param occupancy_layer The input OccupancyLayer
  /// @param block_indices The indices of the CertifiedEsdfLayer which should be updated
  ...
  /// Build an CertifiedEsdfLayer slice from a CertifiedTsdfLayer (incremental) (on GPU)
  /// This function takes the voxels between z_min and z_max in the CertifiedTsdfLayer.
  ...
  /// ESDF is written to.
  /// @param[out] esdf_layer The output CertifiedEsdfLayer
  void integrateSlice(const CertifiedTsdfLayer& tsdf_layer,
                      const std::vector<Index3D>& block_indices, float z_min,
                      float z_max, float z_output, CertifiedEsdfLayer* esdf_layer);

  /// Build an CertifiedEsdfLayer slice from a OccupancyLayer (incremental) (on GPU)
  ...
  /// pixel.
  void convertLayerSliceToDistanceImage(const CertifiedEsdfLayer& layer,
  ...
  /// A parameter getter
  /// The maximum (TSDF) distance at which we call a voxel in the CertifiedTsdfLatyer a
  ...
  /// A parameter getter
  /// The minimum (TSDF) weight at which we call a voxel in the CertifiedTsdfLatyer a
  /// "site". A site is a voxel that is on the surface for the purposes of ESDF
  /// calculation.
 ...
 protected:
  ...
  CertifiedTsdfSiteFunctor getSiteFunctor(const CertifiedTsdfLayer& layer);
  ...
  void sweepBlockBandCombined(device_vector<Index3D>* block_indices,
                              CertifiedEsdfLayer* esdf_layer,
                              float max_squared_distance_vox);
  void computeCertifiedEsdfCombined(const device_vector<Index3D>& blocks_with_sites,
                           CertifiedEsdfLayer* esdf_layer);
  ...
  // Output helper.
  void populateSliceFromLayer(const CertifiedEsdfLayer& layer,
                              const AxisAlignedBoundingBox& aabb,
                              float z_slice_height, float resolution,
                              float unobserved_value, bool image_coordinates,
                              Image<float>* image);

  /// @brief CertifiedTsdfLayer related parameter
