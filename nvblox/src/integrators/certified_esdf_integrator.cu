...
struct CertifiedTsdfSiteFunctor {
  __device__ bool isVoxelObserved(const CertifiedTsdfVoxel& tsdf_voxel) const {
    return tsdf_voxel.weight >= min_weight;
  }

  __device__ bool isVoxelInsideObject(const CertifiedTsdfVoxel& tsdf_voxel) const {
    return tsdf_voxel.distance <= 0.0f;
  }

  __device__ bool isVoxelNearSurface(const CertifiedTsdfVoxel& tsdf_voxel) const {
    return fabsf(tsdf_voxel.distance) <= max_site_distance_m;
  }

  __device__ void updateSquashedExtremumAtomic(const CertifiedTsdfVoxel& tsdf_voxel,
                                               CertifiedTsdfVoxel* current_value) const {
...
CertifiedEsdfIntegrator::~CertifiedEsdfIntegrator() {
  if (cuda_stream_ != nullptr) {
    cudaStreamDestroy(cuda_stream_);
  }
}

float CertifiedEsdfIntegrator::max_distance_m() const { return max_distance_m_; }

float CertifiedEsdfIntegrator::max_site_distance_vox() const {
  return tsdf_max_site_distance_vox_;
}

float CertifiedEsdfIntegrator::min_weight() const { return tsdf_min_weight_; }

void CertifiedEsdfIntegrator::max_distance_m(float max_distance_m) {
  CHECK_GT(max_distance_m, 0.0f);
  max_distance_m_ = max_distance_m;
}

void CertifiedEsdfIntegrator::max_site_distance_vox(float max_site_distance_vox) {
  CHECK_GT(max_site_distance_vox, 0.0f);
  tsdf_max_site_distance_vox_ = max_site_distance_vox;
}

void CertifiedEsdfIntegrator::min_weight(float min_weight) {
  CHECK_GT(min_weight, 0.0f);
  tsdf_min_weight_ = min_weight;
}

float CertifiedEsdfIntegrator::occupied_threshold() const {
  return probabilityFromLogOdds(occupied_threshold_log_odds_);
}

void CertifiedEsdfIntegrator::occupied_threshold(float occupied_threshold) {
...
void CertifiedEsdfIntegrator::integrateBlocks(const LayerType& layer,
  ...
  computeCertifiedEsdfCombined(updated_indices_device_, esdf_layer);
  if (!cleared_block_indices_device_.empty()) {
    computeCertifiedEsdfCombined(cleared_block_indices_device_, esdf_layer);
  }
  compute_timer.Stop();
}

void CertifiedEsdfIntegrator::integrateBlocks(const CertifiedTsdfLayer& tsdf_layer,
                                     const std::vector<Index3D>& block_indices,
                                     CertifiedEsdfLayer* esdf_layer) {
  integrateBlocks<CertifiedTsdfLayer>(tsdf_layer, block_indices, esdf_layer);
}

void CertifiedEsdfIntegrator::integrateBlocks(const OccupancyLayer& occupancy_layer,
                                     const std::vector<Index3D>& block_indices,
                                     CertifiedEsdfLayer* esdf_layer) {
...
template <typename LayerType>
void CertifiedEsdfIntegrator::integrateSlice(const LayerType& layer,
                                    const std::vector<Index3D>& block_indices,
...
void CertifiedEsdfIntegrator::integrateSlice(const CertifiedTsdfLayer& layer,
                                    const std::vector<Index3D>& block_indices,
                                    float z_min, float z_max, float z_output,
                                    CertifiedEsdfLayer* esdf_layer) {
  integrateSlice<CertifiedTsdfLayer>(layer, block_indices, z_min, z_max, z_output,
                            esdf_layer);
}

void CertifiedEsdfIntegrator::integrateSlice(const OccupancyLayer& layer,
                                    const std::vector<Index3D>& block_indices,
                                    float z_min, float z_max, float z_output,
                                    CertifiedEsdfLayer* esdf_layer) {
...
void CertifiedEsdfIntegrator::allocateBlocksOnCPU(
    const std::vector<Index3D>& block_indices, CertifiedEsdfLayer* esdf_layer) {
...
__device__ void clearVoxelDevice(CertifiedEsdfVoxel* voxel,
...
__global__ void markAllSitesCombinedKernel(
    ...
    Index3DDeviceHashMapType<CertifiedEsdfBlock> esdf_block_hash,
  ...
  __shared__ CertifiedEsdfBlock* esdf_block;
...
struct CertifiedTsdfVoxelShared : public CertifiedTsdfVoxel {
  CertifiedTsdfVoxelShared() {}
};
...
template <>
struct SharedVoxel<CertifiedTsdfVoxel> {
  typedef CertifiedTsdfVoxelShared type;
};
...
__global__ void markSitesInSliceCombinedKernel(
  ...
  if (layer_index == 0) {
    ...
    if constexpr (std::is_same<CertifiedTsdfVoxel, VoxelType>::value) {
...
__device__ bool updateSingleNeighbor(const CertifiedEsdfBlock* esdf_block,
                                     const Index3D& voxel_index,
                                     const Index3D& neighbor_voxel_index,
                                     int axis, int direction,
                                     float max_squared_distance_vox,
                                     CertifiedEsdfBlock* neighbor_block) {
  const CertifiedEsdfVoxel* esdf_voxel =
      &esdf_block->voxels[voxel_index.x()][voxel_index.y()][voxel_index.z()];
  CertifiedEsdfVoxel* neighbor_voxel =
      &neighbor_block
...
__device__ bool clearSingleNeighbor(const CertifiedEsdfBlock* esdf_block,
                                    const Index3D& voxel_index,
                                    const Index3D& neighbor_voxel_index,
                                    int axis, int direction,
                                    float max_squared_distance_vox,
...
__global__ void updateNeighborBandsCombinedKernel(
    int i, int num_blocks, Index3DDeviceHashMapType<CertifiedEsdfBlock> block_hash,
  ...
  __shared__ CertifiedEsdfBlock* block_ptr;
  CertifiedEsdfBlock* neighbor_block_ptr = nullptr;
...
__global__ void populateSliceFromLayerKernel(
    Index3DDeviceHashMapType<CertifiedEsdfBlock> block_hash, AxisAlignedBoundingBox aabb,
  ...
  const float voxel_size = block_size / CertifiedEsdfBlock::kVoxelsPerSide;
  ...
  CertifiedEsdfBlock* block_ptr = nullptr;
  ...
  const CertifiedEsdfVoxel* voxel =
      &block_ptr->voxels[voxel_index.x()][voxel_index.y()][voxel_index.z()];
...
__global__ void sweepBlockBandCombinedKernel(
    int num_blocks, Index3DDeviceHashMapType<CertifiedEsdfBlock> block_hash,
    float max_squared_distance_vox, Index3D* block_indices) {
...
__global__ void clearAllInvalidKernel(
    Index3D* block_indices, Index3DDeviceHashMapType<CertifiedEsdfBlock> block_hash,
  ...
  if (esdf_voxel->observed && !esdf_voxel->is_site &&
    ...
    CertifiedEsdfVoxel* neighbor_voxel = nullptr;
...
void CertifiedEsdfIntegrator::sortAndTakeUniqueIndices(
...
// Combined functions.
void CertifiedEsdfIntegrator::updateNeighborBandsCombined(
    device_vector<Index3D>* block_indices, CertifiedEsdfLayer* esdf_layer,
    float max_squared_distance_vox,
  ...
  GPULayerView<CertifiedEsdfBlock> gpu_layer_view = esdf_layer->getGpuLayerView();
  gpu_view.Stop();
...
void CertifiedEsdfIntegrator::sweepBlockBandCombined(
    device_vector<Index3D>* block_indices, CertifiedEsdfLayer* esdf_layer,
    float max_squared_distance_vox) {
  if (block_indices->empty()) {
    return;
...
void CertifiedEsdfIntegrator::computeCertifiedEsdfCombined(
    const device_vector<Index3D>& blocks_with_sites, CertifiedEsdfLayer* esdf_layer) {
...
void CertifiedEsdfIntegrator::clearAllInvalid(
    const std::vector<Index3D>& blocks_to_clear, CertifiedEsdfLayer* esdf_layer,
...
OccupancySiteFunctor CertifiedEsdfIntegrator::getSiteFunctor(
...
CertifiedTsdfSiteFunctor CertifiedEsdfIntegrator::getSiteFunctor(const CertifiedTsdfLayer& layer) {
  CertifiedTsdfSiteFunctor functor;
...
template <typename LayerType>
void CertifiedEsdfIntegrator::markAllSitesCombined(
    const LayerType& layer, const std::vector<Index3D>& block_indices,
    CertifiedEsdfLayer* esdf_layer, device_vector<Index3D>* blocks_with_sites,
  ...
  GPULayerView<CertifiedEsdfBlock> esdf_layer_view = esdf_layer->getGpuLayerView();
  ...
  constexpr int kVoxelsPerSide = CertifiedEsdfBlock::kVoxelsPerSide;
  dim3 dim_threads(kVoxelsPerSide, kVoxelsPerSide, kVoxelsPerSide);
...
template <typename LayerType>
void CertifiedEsdfIntegrator::markSitesInSliceCombined(
    const LayerType& input_layer, const std::vector<Index3D>& block_indices,
    float min_z, float max_z, float output_z, CertifiedEsdfLayer* esdf_layer,
    device_vector<Index3D>* updated_blocks,
    device_vector<Index3D>* cleared_blocks) {
  if (block_indices.empty()) {
    return;
...
void CertifiedEsdfIntegrator::populateSliceFromLayer(
    const CertifiedEsdfLayer& layer, const AxisAlignedBoundingBox& aabb,
  ...
  GPULayerView<CertifiedEsdfBlock> gpu_layer_view = layer.getGpuLayerView();
...
void CertifiedEsdfIntegrator::convertLayerSliceToDistanceImage(
    const CertifiedEsdfLayer& layer, float unobserved_value, float slice_height,
...
void CertifiedEsdfIntegrator::convertLayerSliceToDistanceImage(
  ...
  const float block_size = layer.block_size();
  constexpr int kVoxelsPerSide = VoxelBlock<CertifiedEsdfVoxel>::kVoxelsPerSide;
