...
def animate_slices_with_voxblox_colormap(points, intensity, mesh_file, truncation_distance=3.0, voxel_size=0.02):
    """
    ...
    unique_z = np.sort(np.unique(points[:, 2]))
    if len(unique_z) > 1:
    ...
    def update_slice(slice_index):
        ...
        slice_mask = (points[:, 2] >= z_min) & (points[:, 2] < z_max)
...
def main():
    ...
    animate_slices_with_voxblox_colormap(points, intensity, args.mesh_file, truncation_distance=args.truncation_distance, voxel_size=args.voxel_size)
