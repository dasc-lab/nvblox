...
function PlyMesh(ply; color=:auto, is_trimesh=false)
    ...
    if is_trimesh
        for (i, tri_face) in enumerate(ply["face"]["vertex_indices"])
            p1, p2, p3 = tri_face
...
function draw_mesh!(vis, plymesh::PlyMesh; alpha=1.0)
    mat = MeshCat.MeshPhongMaterial(; vertexColors = 1, color=RGBA(1,1,1,alpha))
